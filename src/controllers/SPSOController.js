...
class SPSOController {
    ...
    async updatePageUnitPrice(req, res) {
        ...
        try {
            ...
            const systemConfig = await SystemConfig.getConfigById(configId, db);
    
            // Cập nhật giá trị pageUnitPrice
            const success = await systemConfig.updatePageUnitPrice(pageUnitPrice, db, configId);
    ...
    async resetHistory(req, res) {
        try {
            const historyLogCollection = db.collection('HistoryLog');
    ...
    async getSystemConfig(req, res) {
        try {
            const snapshot = await db.collection('SystemConfig').get();
